...
test('source imports all 215 runs and preserves local dates', () => {
  const runs = parseRuns(fs.readFileSync('tests/fixtures/runs.csv','utf8')); assert.equal(runs.length,215); assert.equal(runs[0].date,'2026-09-13'); assert.equal(runs[0].seconds,0); assert.equal(runs[1].seconds,1275); assert.equal(paceLabel(runs[1].seconds,runs[1].distance),'36:01');
});
test('Excel repository preserves every source record and dashboard totals', async () => {
  ...
  assert.equal(runs.length, 215);
  const stats = summarize(runs);
  assert.equal(stats.athletes.length, 36);
  assert.equal(Math.round(stats.distance * 100), 124682);
  assert.equal(paceLabel(stats.seconds, stats.timedDistance), '8:25');
