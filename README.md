...
Buka http://localhost:3000. Produksi: `npm run build` lalu `npm start`. Pemeriksaan TypeScript: `npm run check`.
...
Ganti `data/data lari-2026-9-18-9.xlsx` dengan workbook Excel. Worksheet pertama harus memiliki header `Name`, `Date`, `Distance`, `Unit`, dan `Duration` pada baris pertama; kolom boleh diurutkan ulang. Server membaca workbook langsung setiap halaman atau API `/api/runs` dimuat. Duration mendukung waktu Excel, pecahan hari, atau teks `HH:mm:ss`. Date mendukung teks ISO (tanggal lokal dipertahankan) dan sel tanggal Excel. Pace dihitung dari durasi detik dibagi jarak, karena kolom Pace sumber tidak konsisten. Rata-rata pace menggunakan total durasi dibagi total jarak aktivitas dengan durasi valid. Aktivitas tanpa durasi masuk total jarak, tetapi tidak masuk perhitungan pace. Data merupakan snapshot lokal. Workbook tersimpan di luar folder public; API tetap menyajikan data aktivitas. Ekspor hasil filter tetap berupa CSV. Konfigurasi tracing menyertakan workbook dalam build deployment. Jalankan `npm test` untuk verifikasi impor Excel dan perhitungan.
